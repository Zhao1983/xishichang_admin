...
import { getAdvertKind, setAddAdvert, setUploadImage } from '@/api/advert' // 광고용 API 추가(광고이미지, 관련링크, 광고추가)
    ...
    methods: {
        ...
        async setRegisterAdvert() { // 광고 등록
            ...
            if (this.dataForm.imgFile === undefined) {
                showToast(this, '请上传广告图片', 'warning')
                return
            }
            ...
            setAddAdvert(query).then(response => {
                if (response.code === 0) {
                    showToast(this, '操作成功', 'success')
