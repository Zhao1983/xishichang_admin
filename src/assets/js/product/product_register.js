...
export default {
    ...
    methods: {
        ...
        checkExist() { // 상품중복여부 얻기
            return new Promise((resolve, reject) => {
                existProduct(encodeURIComponent(this.dataForm.goodsName), this.dataForm.shopId, 0).then(response => {
                    resolve(response)
                }).catch(error => {
                    reject(error)
                })
        ...
        async setRegister() {
            ...
            // 공급가가 판매가보다 작으면 확인 팝업 로출
            if (parseFloat(this.dataForm.costPrice) < parseFloat(this.dataForm.salesPrice)) {
            ...
            if (this.isOption === 1) { // 옵션상품사용이면
                for (let i = 0; i < this.dataOption.length; i++) {
                    ...
                    // 공급가가 판매가보다 작으면 확인 팝업 로출
                    if (parseFloat(this.dataOption[i].costPrice) < parseFloat(this.dataOption[i].salesPrice)) {
