...
export default {
    ...
    async created() {
        ...
        this.productid = this.$route.params && this.$route.params.subid // 해당 상품 아이디
        this.shopId = this.$route.params && this.$route.params.id // 해당 상품 아이디
    ...
    methods: {
        ...
        checkExist() { // 상품중복여부 얻기
            return new Promise((resolve, reject) => {
                existProduct(encodeURIComponent(this.dataForm.goodsName), this.shopId, this.productid).then(response => {
                    resolve(response)
                }).catch(error => {
                    reject(error)
                })
