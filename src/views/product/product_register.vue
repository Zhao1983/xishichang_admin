...
<div id="product_register">
    <div class="mixin-components-container" v-loading="listLoading">
        <el-form ref="prodForm" :model="dataForm" :rules="rules" class="form-container">
            ...
            <el-card class="box-card" style="margin-bottom: 10px;">
                ...
                <el-row>
                    ...
                    <el-col :span="24">
                        <el-col :span="8">
                            <!-- 점포 -->
                            <el-form-item prop="shopName" label-width="15%" label="所属商户: " class="postInfo-container-item">
                                <el-input id="shopName" ref="shopName" v-model="dataForm.shopName" size="small" placeholder="请输入所属商户" style="width: 50%;" :readonly="true" />
                                <el-button type="primary" icon="el-icon-plus" size="small" @click="setShowShopDialog" style="margin-left: 5px;">绑定</el-button>
                                <span style="color: #999999; font-size: 12px; margin-left: 10px;">(只能绑定一个商户,红色表示已下架商户)</span>
                            </el-form-item>
                        </el-col>
                        <el-col :span="8">
                            <!-- 중량 -->
                            <el-form-item prop="goodsWeight" label-width="25%" label="商品重量(kg): " class="postInfo-container-item">
                    ...
                    <el-col :span="24">
                        <el-col :span="8">
                            ...
                            <el-form-item prop="tagName" label-width="15%" label="所属标签: " class="postInfo-container-item">
                                <el-input id="tagName" ref="tagName" v-model="dataForm.tagName" size="small" placeholder="请输入所属标签" style="width: 50%;" :readonly="true" />
                        ...
                        <el-col :span="8">
                            <!-- 상품 분류 -->
                            <el-form-item prop="typeName" label-width="25%" label="所属分类: " class="postInfo-container-item">
                                <el-input id="typeName" ref="typeName" v-model="dataForm.typeName" size="small" placeholder="请输入所属分类" style="width: 46%;" :readonly="true" />
            ...
            <el-card class="box-card" style="margin-bottom: 10px;">
                <el-row>
                    <!-- 원가 -->
                    <el-col :span="4">
                        <el-form-item label-width="29.5%" label="原价: " class="postInfo-container-item">
                            <el-input class="color_prop" id="originalPrice" size="small" ref="originalPrice" type="text" v-model="dataForm.originalPrice" placeholder="原价" />
                        </el-form-item>
                    </el-col>
                    <!-- 판매가 -->
                    <el-col :span="4">
                        ...
                        <el-col :span="17">
                            <el-form-item prop="discountPercent" label="" class="postInfo-container-item">
                                <el-input v-if="dataForm.isPercent === true" size="small" id="discountPercent" ref="discountPercent" type="text" v-model="dataForm.discountPercent" placeholder="折扣比例" />
                                <el-input v-else id="discountPercent" ref="discountPercent" size="small" type="text" v-model="dataForm.discountPercent" placeholder="折扣比例" :disabled="true" />
                            </el-form-item>
                        </el-col>
                    </el-col>
                    <!-- 공급가 -->
                    <el-col :span="4">
                        <el-form-item prop="costPrice" label-width="30%" label="供货价: " class="postInfo-container-item">
                            <el-input class="color_prop" id="costPrice" ref="costPrice" size="small" type="text" v-model="dataForm.costPrice" placeholder="供货价" />
                        </el-form-item>
                    </el-col>
                    <!-- 가상판매량 -->
                    <el-col :span="4">
                        ...
                        <el-table :data="dataOption" border fit highlight-current-row style="width: 100%;" v-show="isOption === 1">
                            <el-table-column label="图片" align="center" width="120%;">
                                <template slot-scope="{row, $index}">
                                    <input type="file" name="" :ref="'imageInput-' + $index" value="" @change="setChangeOptionImage(row, $index)" style="display: none;">
                            ...
                            <el-table-column label="销售价" align="center" width="125%;">
                                <template slot-scope="{row, $index}">
                                    <el-popover placement="top-start" title="" width="170" trigger="manual" content="注意销售价低于进价" v-model="row.isVisiblePriceToolTip">
                                        <el-input slot="reference" v-if="row.isPercent" class="color_prop" size="small" :id="'salesPrice-' + $index" v-model="row.salesPrice" type="text" :ref="'salesPrice-' + $index" placeholder="" :readonly="true" />
                                        <el-input slot="reference" v-else class="color_prop" size="small" :id="'salesPrice-' + $index" v-model="row.salesPrice" type="text" :ref="'salesPrice-' + $index" placeholder="" />
                                    </el-popover>
                                </template>
                            </el-table-column>
                            <el-table-column label="折扣比例" align="center" width="125%;">
                                <template slot-scope="{row, $index}">
                                    ...
                                    <el-col :span="20">
                                        <el-input v-if="row.isPercent" size="small" :id="'discountPercent-' + $index" v-model="row.discountPercent" type="text" :ref="'discountPercent-' + $index" placeholder="" />
                                        <el-input v-else :id="'discountPercent-' + $index" size="small" v-model="row.discountPercent" type="text" :ref="'discountPercent-' + $index" placeholder="" :disabled="true" />
                                    </el-col>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="重量(kg)" align="center" width="125%;">
                                <template slot-scope="{row, $index}">
                                    <el-input :id="'sizeWeight-' + $index" v-model="row.sizeWeight" size="small" type="text" :ref="'sizeWeight-' + $index" placeholder="" />
                                </template>
                            </el-table-column>
                            <el-table-column label="包装费" align="center" width="170%;">
                                <template slot-scope="{row, $index}">
                                    ...
                                    <el-col :span="20">
                                        <el-input v-if="row.isPackage === true" size="small" :id="'packageNum-' + $index" v-model="row.packageInfo.num" type="text" :ref="'packageNum-' + $index" placeholder="" style="margin-bottom: 10px;" />
                                        <el-select v-if="row.isPackage === true" size="small" v-model="row.packageInfo.name" placeholder="" clearable>
                                            <el-option key="" label="" value="" />
                                            <el-option v-for="item in row.listPackage" :key="item.name" :label="item.name" :value="item.name" />
                                        </el-select>
                                        <el-input v-if="row.isPackage === false" size="small" :id="'packageNum-' + $index" v-model="row.packageInfo.num" type="text" :ref="'packageNum-' + $index" placeholder="" style="margin-bottom: 10px;" :disabled="true" />
                                        <el-select v-if="row.isPackage === false" size="small" v-model="row.packageInfo.name" placeholder="" disabled>
                                            <el-option key="" label="" value="" />
                                            <el-option v-for="item in row.listPackage" :key="item.name" :label="item.name" :value="item.name" />
                                        </el-select>
                                    </el-col>
                            ...
                            <el-table-column label="虚拟销量" align="center" width="125%;">
                                <template slot-scope="{row, $index}">
                                    <el-input :id="'visualSalesNum-' + $index" size="small" v-model="row.visualSalesNum" type="text" :ref="'visualSalesNum-' + $index" placeholder="" />
                                </template>
                            </el-table-column>
                ...
                <el-row v-show="!dataForm.postageFreeStatus">
                    ...
                    <el-col :span="3">
                        <el-form-item label-width="23%" label="每" class="postInfo-container-item">
                            <el-input id="packageNumber" ref="packageNumber" size="small" v-if="dataForm.isPackage" v-model="dataForm.packageDto.num" type="text" placeholder="" style="width: 80%;" />
                            <el-input id="packageNumber" ref="packageNumber" size="small" v-else type="text" v-model="dataForm.packageDto.num" placeholder="" style="width: 80%;" :disabled="true" />
                ...
                <el-row v-for="(item, index) in goodsProp" :key="item.key">
                    ...
                    <el-col :span="3">
                        <el-form-item label-width="22%" label="" class="postInfo-container-item">
                            <el-input :id="'key-' + index" :ref="'key-' + index" size="small" v-model="item.keys" type="text" placeholder="" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label-width="2%" label="" class="postInfo-container-item">
                            <el-input :id="'value-' + index" :ref="'value-' + index" size="small" type="text" v-model="item.value" placeholder="" style="width: 40%;" />
                    ...
                    <el-col :span="24">
                        <el-form-item label-width="4.5%" label="" class="postInfo-container-item">
                            <ul ref="dragElement" style="list-style-type: none; padding: 0; display: flex;">
                                <Thumbnails v-for="img in listMainImage" :key="img.imgUri" :imageUrl="img.imgUri" :objData="listMainImage" :img="img.imgUri" />
                            </ul>
                        </el-form-item>
                    </el-col>
                </el-row>
        ...
        <el-row>
            <router-link :to="'/product/product_list'" style="position: fixed; right: 120px; bottom: 50px; z-index: 10;">
                <el-button type="danger">取消</el-button>
            </router-link>
            <el-button type="primary" @click="setRegister" style="position: fixed; right: 40px; bottom: 50px; z-index: 10;">保存</el-button>
        </el-row>
    </div>
    ...
    <el-dialog v-el-drag-dialog title="保存" :visible.sync="isShowShopDialog" :close-on-click-modal="false">
        <div class="filter-container" style="margin-bottom: 10px;">
            <el-row>
                <el-col :span="15">
                    <el-input id="searchShopName" size="small" ref="searchShopName" v-model="searchShopName" placeholder="请以商户名称进行搜索" @keyup.enter.native="setSearchShop" />
                </el-col>
                <el-col :span="9">
...
<script src="@/assets/js/product/product_register.js"></script>
